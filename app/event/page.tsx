...
// /* ============================================================================
//    FLOW TIMELINE — organic, non-repeating diagonal curve (golden-angle offsets)
//    threaded through a Catmull-Rom spline, so it is smooth by construction and
//    mathematically cannot contain a sharp kink.
// ============================================================================ */

// type Pt = { x: number; y: number };
// type Arrow = { x: number; y: number; angle: number };
// type NodePos = { x: number; y: number; item: ScheduleItem };
...
// /** Diagonal top-left -> bottom-right drift, wobbled with golden-angle phases
//  *  so no two segments echo each other — a genuinely non-repeating path. */
// function buildDiagonalPoints(n: number, width: number, height: number): Pt[] {
//   if (n <= 0) return [];
//   const points: Pt[] = [];
//   for (let i = 0; i < n; i++) {
//     const t = n === 1 ? 0.5 : i / (n - 1);
//     const baseX = width * (0.08 + t * 0.84);
//     const baseY = height * (0.06 + t * 0.88);
//     const phase = i * GOLDEN_ANGLE;
//     const wobbleX =
//       Math.sin(phase) * width * 0.1 + Math.sin(phase * 0.31 + 1.3) * width * 0.045;
//     const wobbleY = Math.cos(phase * 0.63 + 0.4) * height * 0.032;
//     points.push({
//       x: clamp(baseX + wobbleX, width * 0.04, width * 0.96),
//       y: clamp(baseY + wobbleY, height * 0.03, height * 0.97),
//     });
//   }
//   return points;
// }
...
//   toggleBookmark: (id: number, e?: React.MouseEvent) => void;
...
//   const desiredHeight = Math.max(480, items.length * 118);
...
//   const pathD = useMemo(
//     () => (size ? catmullRomPath(buildDiagonalPoints(items.length, size.w, size.h)) : ""),
//     [size, items.length],
//   );

//   useEffect(() => {
//     const path = pathRef.current;
//     if (!path || !pathD || items.length === 0) {
//       setArrows([]);
//       setNodes([]);
//       return;
//     }
...
//     for (let len = margin; len < total - margin; len += spacing) {
//       const p = path.getPointAtLength(len);
//       const p2 = path.getPointAtLength(Math.min(len + 1, total));
//       const angle = (Math.atan2(p2.y - p.y, p2.x - p.x) * 180) / Math.PI;
//       nextArrows.push({ x: p.x, y: p.y, angle });
//     }
//     setArrows(nextArrows);

//     const nextNodes: NodePos[] = items.map((item, i) => {
...
//   return (
//     <>
//       <div className="relative hidden lg:block" style={{ height: desiredHeight }}>
...
//                 strokeDasharray="1 9"
//                 fill="none"
//                 className="text-[#18181B]/50 dark:text-white/35"
//               />
//               {arrows.map((a, i) => (
...
//               const cardWidth = 300;
//               const left = clamp(x - cardWidth / 2, 12, Math.max(12, size.w - cardWidth - 12));
//               const showBelow = y < size.h * 0.62;
...
//                 >
//                   <button
//                     onClick={() => setPinnedId(isPinned ? null : item.id)}
//                     className={`relative z-10 flex items-center gap-1.5 whitespace-nowrap rounded-full border-2 px-3 py-1.5 text-[10px] font-black shadow-[3px_3px_0px_0px_#18181B] transition-all hover:-translate-y-0.5 dark:shadow-[3px_3px_0px_0px_#FFF] ${
//                       item.isKeyMilestone
//                         ? "border-[#FF3B30] bg-[#FF3B30] text-white"
//                         : "border-[#18181B] bg-white text-[#18181B] dark:border-white dark:bg-zinc-900 dark:text-white"
//                     }`}
//                   >
//                     {item.isKeyMilestone && (
//                       <span className="absolute -right-1 -top-1 h-2.5 w-2.5 animate-pulse rounded-full border border-white bg-[#FDE047]" />
//                     )}
...
//                   <div
//                     className={`absolute z-30 w-[300px] rounded-2xl border-2 border-[#18181B] bg-white p-4 text-left shadow-[6px_6px_0px_0px_#18181B] transition-all duration-150 dark:border-white dark:bg-zinc-900 dark:shadow-[6px_6px_0px_0px_#FFF] ${
//                       isPinned
//                         ? "visible opacity-100"
//                         : "invisible opacity-0 group-hover/node:visible group-hover/node:opacity-100"
//                     }`}
//                     style={{ left: left - x, ...(showBelow ? { top: 30 } : { bottom: 30 }) }}
//                   >
//                     <div className="flex flex-wrap items-center gap-2">
//                       <span
//                         className={`rounded-md border border-[#18181B] px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${item.badgeColor}`}
//                       >
//                         {item.category}
//                       </span>
//                       <span className="rounded-md border border-[#18181B] bg-zinc-100 px-2 py-0.5 text-[9px] font-black text-zinc-700 dark:border-white dark:bg-zinc-800 dark:text-zinc-200">
//                         {item.day}
//                       </span>
//                     </div>

//                     <h4 className="mt-2 text-sm font-black leading-snug text-[#18181B] dark:text-white">
//                       {item.title}
//                     </h4>

//                     <p className="mt-1.5 text-[11px] font-bold leading-relaxed text-zinc-600 dark:text-zinc-400 line-clamp-2">
//                       {item.desc}
//                     </p>
...
//                         <MapPin size={11} className="text-[#26A192]" />
//                         {item.location}
//                       </span>
//                     </div>

//                     <div className="mt-3 flex items-center gap-2 border-t-2 border-zinc-100 pt-3 dark:border-zinc-800">
//                       <button
//                         onClick={(e) => {
//                           e.stopPropagation();
//                           onOpenDetails(item);
//                         }}
//                         className="flex flex-1 items-center justify-center gap-1 rounded-xl border-2 border-[#18181B] bg-[#FDE047] px-3 py-1.5 text-[10px] font-black text-[#18181B] shadow-[2px_2px_0px_0px_#18181B] dark:border-white"
//                       >
//                         Full details <ChevronRight size={12} />
//                       </button>
//                       <button
//                         onClick={(e) => toggleBookmark(item.id, e)}
//                         className={`rounded-xl border-2 border-[#18181B] p-2 dark:border-white ${
//                           isBookmarked ? "bg-[#FF3B30] text-white" : "bg-white text-[#18181B] dark:bg-zinc-800 dark:text-white"
//                         }`}
//                       >
...
//         <div className="pointer-events-none absolute left-4 top-3 bottom-3 w-0.5 border-l-2 border-dotted border-[#18181B]/40 dark:border-white/30" />
//         {items.map((item) => {
//           const IconComponent = item.icon;
//           const isBookmarked = bookmarkedIds.includes(item.id);
//           return (
...
//               <div
//                 className={`absolute left-0 top-0.5 flex h-8 w-8 items-center justify-center rounded-xl border-2 shadow-[2px_2px_0px_0px_#18181B] dark:shadow-[2px_2px_0px_0px_#FFF] ${
//                   item.isKeyMilestone
//                     ? "border-[#FF3B30] bg-[#FF3B30] text-white"
//                     : "border-[#18181B] bg-[#FDE047] text-[#18181B] dark:border-white"
//                 }`}
//               >
...
//                 className="w-full rounded-2xl border-2 border-[#18181B] bg-white p-4 text-left shadow-[4px_4px_0px_0px_#18181B] transition-all active:translate-y-0.5 dark:border-white dark:bg-zinc-900 dark:shadow-[4px_4px_0px_0px_#FFF]"
//               >
//                 <div className="flex flex-wrap items-center gap-1.5">
//                   <span className={`rounded-md border border-[#18181B] px-1.5 py-0.5 text-[9px] font-black uppercase ${item.badgeColor}`}>
//                     {item.category}
//                   </span>
//                   <span className="text-[10px] font-black text-[#FF3B30]">{item.time}</span>
//                 </div>
//                 <h4 className="mt-1.5 text-sm font-black text-[#18181B] dark:text-white">{item.title}</h4>
//                 <div className="mt-2 flex items-center justify-between text-[10px] font-bold text-zinc-500 dark:text-zinc-400">
//                   <span className="flex items-center gap-1">
//                     <MapPin size={11} className="text-[#26A192]" /> {item.location}
//                   </span>
//                   <Bookmark
//                     size={13}
//                     onClick={(e) => toggleBookmark(item.id, e)}
//                     fill={isBookmarked ? "currentColor" : "none"}
//                     className={isBookmarked ? "text-[#FF3B30]" : ""}
//                   />
//                 </div>
//               </button>
//             </div>
...
//   const [scrubberHour, setScrubberHour] = useState<number>(8);
...
//     e?.stopPropagation();
//     navigator.clipboard.writeText(`Join me at "${item.title}" (${item.time}) at ${item.location}!`);
//     setCopied(true);
...
//         <DevfolioArt.BlueFin />
//       </div>
//       <div className="pointer-events-none absolute right-8 top-20 hidden w-28 animate-float xl:block z-20 opacity-80" style={{ animationDelay: "1.2s" }}>
//         <DevfolioArt.YellowSmileBadge />
//       </div>
//       <div className="pointer-events-none absolute left-10 top-[650px] hidden w-24 animate-float xl:block z-20 opacity-80" style={{ animationDelay: "1.6s" }}>
...
//               {[
//                 { label: "Sprint Status", value: "Active Sprint", icon: Radio, accent: "#FF3B30", bg: "bg-[#FF3B30]/10", pulse: true },
//                 { label: "Sprint Duration", value: "24 Hours Non-Stop", icon: Clock, accent: "#18181B", bg: "bg-[#FDE047]/20" },
//                 { label: "Mentoring Track", value: "2 Rounds", icon: Users, accent: "#26A192", bg: "bg-[#26A192]/20" },
//                 { label: "Evaluations", value: "2 Elimination Rounds", icon: Trophy, accent: "#81A2EF", bg: "bg-[#81A2EF]/20" },
//               ].map((s) => (
//                 <div key={s.label} className={`flex flex-col items-center justify-center gap-1 rounded-2xl border border-[#18181B] p-3 transition-transform hover:-translate-y-0.5 dark:border-white/20 ${s.bg}`}>
//                   <span className="flex items-center gap-1 text-[10px] font-black uppercase" style={{ color: s.accent }}>
//                     <s.icon size={12} className={s.pulse ? "animate-pulse" : ""} />
//                     {s.label}
//                   </span>
//                   <span className="text-sm font-black sm:text-base" style={{ color: s.accent === "#18181B" ? undefined : s.accent }}>
//                     {s.value}
//                   </span>
//                 </div>
//               ))}
...
//               </div>
//               <div>
//                 <h3 className="text-sm font-black uppercase tracking-wider">Interactive Time Scrubber</h3>
//                 <p className="text-xs font-bold text-zinc-500 dark:text-zinc-400">
//                   Drag across the bar to jump to Hour {scrubberHour} of the sprint
//                 </p>
//               </div>
//             </div>
...
//             <div className="relative h-6 w-full overflow-hidden rounded-2xl border-2 border-[#18181B] bg-zinc-100 dark:border-white dark:bg-zinc-800">
//               <div className="absolute bottom-0 left-0 top-0 border-r-2 border-[#18181B] bg-[#FF3B30] transition-all duration-75 dark:border-white" style={{ width: `${scrubberPercent}%` }} />
//               <div className="pointer-events-none absolute inset-0 flex items-center justify-between px-3 text-[9px] font-black text-zinc-600 dark:text-zinc-300">
...
//             <div
//               className="pointer-events-none absolute top-2 flex h-8 w-8 -translate-y-1 items-center justify-center rounded-xl border-2 border-[#18181B] bg-[#FDE047] text-[#18181B] shadow-[2px_2px_0px_0px_#18181B] transition-all duration-75 dark:border-white"
//               style={{ left: `calc(${clamp(scrubberPercent, 2, 96)}% - 16px)` }}
//             >
...
//             <div className="mt-3 flex justify-between text-[10px] font-black text-zinc-500 dark:text-zinc-400">
...
//         <section className="mt-8 rounded-3xl border-2 border-[#18181B] bg-white p-4 shadow-[6px_6px_0px_0px_#18181B] dark:border-white dark:bg-zinc-900 dark:shadow-[6px_6px_0px_0px_#FFF] sm:p-6">
...
//                   className="w-full rounded-2xl border-2 border-[#18181B] bg-white py-2 pl-10 pr-4 text-xs font-bold text-[#18181B] outline-none transition-all placeholder-zinc-400 focus:ring-2 focus:ring-[#FDE047] dark:border-white dark:bg-zinc-800 dark:text-white dark:placeholder-zinc-500"
//                 />
//               </div>
...
//                   showBookmarkedOnly ? "bg-[#FF3B30] text-white" : "bg-white text-[#18181B] dark:bg-zinc-800 dark:text-white"
//                 }`}
//               >
...
//           <div className="mt-4 flex flex-wrap items-center gap-2 border-t-2 border-zinc-100 pt-4 dark:border-zinc-800">
//             <span className="mr-2 text-xs font-black uppercase tracking-wider text-zinc-400">Filter Track:</span>
//             {CATEGORIES.map((cat) => (
//               <button
//                 key={cat}
...
//               <p className="mt-1 text-xs font-bold text-zinc-500">Try resetting your search filters or bookmarked items.</p>
//               <button
//                 onClick={() => {
//                   setSelectedDay("All");
...
//                 return (
//                   <div key={cat} className="rounded-3xl border-2 border-[#18181B] bg-white p-5 shadow-[6px_6px_0px_0px_#18181B] dark:border-white dark:bg-zinc-900 dark:shadow-[6px_6px_0px_0px_#FFF]">
//                     <div className="flex items-center justify-between border-b-2 border-[#18181B] pb-3 dark:border-white">
//                       <h4 className="text-sm font-black uppercase tracking-wider">{cat}</h4>
//                       <span className="rounded-full bg-[#FDE047] px-2 py-0.5 text-xs font-black text-[#18181B]">{categoryItems.length}</span>
//                     </div>

//                     <div className="mt-4 space-y-4">
//                       {categoryItems.length === 0 ? (
//                         <p className="py-4 text-center text-xs font-bold text-zinc-400">No events in this category</p>
//                       ) : (
...
//                                 <span className="text-[10px] font-black text-[#FF3B30]">{item.time}</span>
//                                 <span className="text-[10px] font-black uppercase text-zinc-500">{item.day}</span>
//                               </div>
//                               <h5 className="mt-1 flex items-center gap-1.5 text-sm font-black text-[#18181B] dark:text-white">
//                                 <IconComponent size={14} className="text-[#FF3B30]" />
//                                 {item.title}
//                               </h5>
//                               <p className="mt-2 line-clamp-2 text-[11px] font-bold text-zinc-600 dark:text-zinc-400">{item.desc}</p>
//                             </div>
//                           );
//                         })
//                       )}
...
//                 <thead className="border-b-2 border-[#18181B] bg-[#FDE047] font-black uppercase tracking-wider text-[#18181B] dark:border-white">
//                   <tr>
...
//                     <tr key={item.id} onClick={() => setActiveModalItem(item)} className="cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
//                       <td className="whitespace-nowrap p-4 font-black text-[#FF3B30]">
//                         {item.time} ({item.day})
//                       </td>
//                       <td className="p-4 font-black text-[#18181B] dark:text-white">{item.title}</td>
//                       <td className="p-4">
//                         <span className={`rounded-md border border-[#18181B] px-2 py-0.5 text-[10px] font-black uppercase ${item.badgeColor}`}>{item.category}</span>
//                       </td>
...
//             <div className="relative w-full max-w-2xl rounded-3xl border-2 border-[#18181B] bg-white p-6 text-left shadow-[10px_10px_0px_0px_#18181B] dark:border-white dark:bg-zinc-900 dark:shadow-[10px_10px_0px_0px_#FFF] sm:p-8">
//               <button
//                 onClick={() => setActiveModalItem(null)}
//                 className="absolute right-5 top-5 rounded-xl border-2 border-[#18181B] bg-zinc-100 p-2 text-[#18181B] transition-all hover:bg-[#FF3B30] hover:text-white dark:border-white dark:bg-zinc-800 dark:text-white"
//               >
//                 <X size={18} />
//               </button>

//               <div className="mb-3 flex flex-wrap items-center gap-2">
//                 <span className={`rounded-md border border-[#18181B] px-2.5 py-0.5 text-[10px] font-black uppercase ${activeModalItem.badgeColor}`}>
...
//               <h2 className="text-2xl font-black text-[#18181B] dark:text-white sm:text-3xl">{activeModalItem.title}</h2>

//               <p className="mt-3 text-xs font-bold leading-relaxed text-zinc-600 dark:text-zinc-300 sm:text-sm">{activeModalItem.desc}</p>

//               <div className="mt-6 rounded-2xl border-2 border-[#18181B] bg-[#FAF9F5] p-4 dark:border-white dark:bg-zinc-800">
//                 <h4 className="mb-2 text-xs font-black uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
...
//               <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t-2 border-zinc-100 pt-4 dark:border-zinc-800">
...
//                       bookmarkedIds.includes(activeModalItem.id) ? "bg-[#FF3B30] text-white" : "bg-[#FDE047] text-[#18181B]"
//                     }`}
//                   >
...
//             <div>
//               <span className="rounded-md bg-[#18181B] px-2.5 py-0.5 text-[10px] font-black uppercase text-[#FDE047]">Official Venue & Campus</span>
//               <h3 className="mt-3 text-2xl font-black text-[#18181B] dark:text-white sm:text-3xl">CGC University Campus</h3>
//               <p className="mt-1 text-xs font-bold leading-relaxed text-zinc-800 dark:text-zinc-300 sm:text-sm">
